...
### put your scene path ###
test_scene = "replica_v1/apartment_0/habitat/mesh_semantic.ply"  # 要加載的場景路徑, 包含了三維model的網格數據

# 定義了模擬環境的配置參數, 包括場景路徑, agent的初始位置, 感測器高度, 分辨率等等 
sim_settings = {
...
# 用於執行動作(forward, 轉向), 並且記錄了相機的位置和方向
...
parser.add_argument('-f', '--floor', type=int, default=1)  # 設置agent在第幾層樓, 預設為1
...
if args.floor == 1:     # (0,0,0)是1樓, (0,1,0) 是2樓 
    agent_state.position = np.array([0.0, 0.0, 0.0])  # agent in world space
    
# command line: python3 load.py -f 2    -> agent一開始位置在2樓
elif args.floor == 2:
...
while True:
    ...
    if keystroke == ord(FORWARD_KEY):   # if按w -> 向前走
        action = "move_forward"
        navigateAndSee(action, data_root)
