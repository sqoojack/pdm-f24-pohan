...
class Projection(object):
    ...
    def top_to_front(self, theta=0, phi=0, gamma=0, dx=0, dy=0, dz=0, fov=90):      
        ...
        for point in self.points:
            ...
            front_point_3D[2] += 0.8    # 調整 Z 軸，將投影點推遠
            ...
            if front_point_3D[2] > 0:   # check if point is in front of the camera
                ...
                print("Projected pixel coordinates: ", pixel_2D)    # debug
                
                if 0 <= pixel_2D[0] < img_size and 0 <= pixel_2D[1] < img_size:
                    new_pixels.append(pixel_2D) # add the projected point to the list
                else:
                    print(f"Point {pixel_2D} is out of bounds.")
    ...
    def show_image(self, new_pixels, img_name='projection.png', color=(0, 0, 255), alpha=0.3):
        if len(new_pixels) > 0:
            new_pixels = np.array(new_pixels, dtype=np.int32)
            print("Filling pixels: ", new_pixels)  # Debug 打印即將填充的點
...
if __name__ == "__main__":
    ...
    parser.add_argument("--photo", type=float, default=1)
