...
class Projection(object):
    ...
    def show_image(self, new_pixels, img_name='projection.png', color=(0, 0, 255), alpha=0.4):
        ...
        # 確保 new_pixels 是正確的 numpy array，並強制轉換為 float32 或 int32
        new_pixels = np.array(new_pixels, dtype=np.float32)

        # 檢查 new_pixels 是否有足夠的點
        if new_pixels.shape[0] < 3:
            raise ValueError("new_pixels 至少需要包含三個點以形成多邊形。")

        # 使用 cv2.convexHull
        hull = cv2.convexHull(new_pixels).astype(np.int32)

        # 填充多邊形並顯示
        new_image = cv2.fillPoly(
            self.image.copy(), [hull], color)
